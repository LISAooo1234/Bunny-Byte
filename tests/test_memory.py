...
from bunnybyte.features.memory import (
    LayeredMemory,
    append_to_daily_log,
    build_dream_prompt,
...
def test_durable_memory_index_and_topic_notes_are_loaded_and_retrieved(tmp_path):
    ...
    (topics_dir / "project-conventions.md").write_text(
        ...
        "- Preserve local agent state under .bunnybyte/.\n",
        encoding="utf-8",
    )
...
def test_dream_prompt_targets_repo_local_memory_assets(tmp_path):
    ...
    prompt = build_dream_prompt(memory_root, transcript_dir=str(tmp_path / ".bunnybyte" / "sessions"), session_ids=["s1", "s2"])
...
def test_dream_prompt_uses_four_phase_filesystem_maintenance_flow(tmp_path):
    memory_root = tmp_path / ".bunnybyte" / "memory"
    transcript_dir = tmp_path / ".bunnybyte" / "sessions"
