...
from bunnybyte.evaluation.metrics import (
    _provider_profile,
...
def test_provider_profile_uses_project_toml_before_legacy_bunnybyte_env(tmp_path, monkeypatch):
    ...
    with patch.dict(
        os.environ,
        {
            "BUNNYBYTE_DEEPSEEK_API_KEY": "sk-legacy-deepseek",
            "BUNNYBYTE_DEEPSEEK_MODEL": "legacy-deepseek-model",
            "BUNNYBYTE_DEEPSEEK_API_BASE": "https://legacy.deepseek.example/anthropic",
        },
        clear=True,
    ):
...
def test_write_benchmark_core_report_marks_resume_safe_metrics(tmp_path):
    ...
    report_path = tmp_path / "docs" / "metrics" / "bunnybyte-benchmark-core-report.md"
    ...
    assert "可复用的评估指标" in report_text
