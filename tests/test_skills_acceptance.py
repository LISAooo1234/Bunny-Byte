...
from bunnybyte.features import skills as skillslib
...
def test_prompt_documents_project_skill_frontmatter_contract(tmp_path):
    ...
    prompt = agent.prompt("Create .bunnybyte/skills/audit/SKILL.md")

    assert "When creating BunnyByte skill files" in prompt
    assert ".bunnybyte/skills/<name>/SKILL.md" in prompt
...
---
Use target $ARGUMENTS from ${BUNNYBYTE_SKILL_DIR}.
""",
        encoding="utf-8",
    )
...
def test_memory_slash_commands_use_kairos_assets(tmp_path):
    ...
    (tmp_path / ".bunnybyte" / "memory" / "MEMORY.md").write_text(
...
def test_dream_slash_command_consolidates_daily_log_into_memory_files(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            '<tool>{"name":"write_file","args":{"path":".bunnybyte/memory/topics/user-preferences.md","content":"# User Preferences\\n\\n## Notes\\n- Prefers concise reports.\\n"}}</tool>',
    ...
    assert "User preferences" in (tmp_path / ".bunnybyte" / "memory" / "MEMORY.md").read_text(encoding="utf-8")
    assert "Prefers concise reports" in (tmp_path / ".bunnybyte" / "memory" / "topics" / "user-preferences.md").read_text(encoding="utf-8")
...
def test_dream_retries_after_false_block_and_still_consolidates(tmp_path):
    agent = build_agent(
        tmp_path,
        [
            "<final>Blocked: I need to use write tools to consolidate memory, but this chat turn only permits returning one tool call or one final answer.</final>",
            '<tool>{"name":"read_file","args":{"path":".bunnybyte/memory/MEMORY.md","start":1,"end":50}}</tool>',
            '<tool>{"name":"patch_file","args":{"path":".bunnybyte/memory/MEMORY.md","old_text":"# Durable Memory Index\\n\\n_Empty. `/remember` writes a daily log entry; `/dream` consolidates logs into topic files and adds entries here._\\n","new_text":"# Durable Memory Index\\n\\n- [User Preferences](topics/user-preferences.md): User preferences\\n"}}</tool>',
            '<tool>{"name":"write_file","args":{"path":".bunnybyte/memory/topics/user-preferences.md","content":"# User Preferences\\n\\n## Notes\\n- 我是一个男生\\n"}}</tool>',
            "<final>Dream consolidation complete.</final>",
        ],
    )
    handle_repl_command(agent, "/remember 我是一个男生")

    handled, should_exit, output = handle_repl_command(agent, "/dream")

    assert handled is True
    assert should_exit is False
    assert output == "Dream consolidation complete."
    assert "User preferences" in (tmp_path / ".bunnybyte" / "memory" / "MEMORY.md").read_text(encoding="utf-8")
    assert "我是一个男生" in (tmp_path / ".bunnybyte" / "memory" / "topics" / "user-preferences.md").read_text(encoding="utf-8")


def test_skill_frontmatter_metadata_and_argument_substitution(tmp_path):
    skill_dir = tmp_path / ".bunnybyte" / "skills" / "audit"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: audit
...
---
Audit ${target} with $ARGUMENTS in ${BUNNYBYTE_SKILL_DIR}.
""",
        encoding="utf-8",
    )
...
def test_fork_skill_runs_in_isolated_session_and_records_completion(tmp_path):
    skill_dir = tmp_path / ".bunnybyte" / "skills" / "inspect"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: inspect
...
def test_skill_allowed_tools_restricts_inline_execution(tmp_path):
    skill_dir = tmp_path / ".bunnybyte" / "skills" / "readonly"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: readonly
...
def test_disable_model_invocation_skill_returns_expanded_prompt(tmp_path):
    skill_dir = tmp_path / ".bunnybyte" / "skills" / "template"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: template
...
def test_cli_lists_skills_without_calling_model(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "bunnybyte", "--cwd", str(tmp_path)],
        input="/skills\n/exit\n",
        text=True,
        capture_output=True,
        env=env,
