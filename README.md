...
- **原生工具协议**：优先使用 OpenAI Responses / Anthropic Messages 的原生 tool calling；本地只执行 provider 返回的结构化工具调用。旧的 `<tool>/<final>` 文本协议仅作为不支持原生工具的 legacy fallback。
...
| 原生工具调用        | 优先使用 OpenAI / Anthropic provider-native tool calling；legacy 文本协议 parser 仅用于不支持原生工具的后端和旧测试。 |
