...
Bunny Byte 跑在本地仓库里，接上一个模型 provider，就能读代码、跑命令、改文件、
...
Bunny Byte 是一个本地终端里的 coding agent，运行在你的仓库上下文里。一次 agent 运行会被拆成几个可观察的部分：
...
- **session / run evidence**：对话、事件流、trace、report 都写到本地 `.bunnybyte/`。
- **memory / dream**：把 daily log 整理成长期 topic，下次 session 可以继续用。

Bunny Byte 关注本地 coding agent 的工程边界：配置清楚、任务能续接、结果能复盘。
...
| --- | --- |
| ![bunnybyte TUI 工具表](assets/screenshots/bunnybyte-tui-tools.png) | ![bunnybyte TUI skills 和 help](assets/screenshots/bunnybyte-tui-skills-help.png) |

| Memory 和 durable topics | Slash command 工作区 |
| --- | --- |
| ![bunnybyte TUI memory 和 skills](assets/screenshots/bunnybyte-tui-memory-skills.png) | ![bunnybyte TUI slash command 补全](assets/screenshots/bunnybyte-tui-latest.png) |
...
Bunny Byte 启动前先解析一个 **provider profile**。一个 profile 主要由四项组成：
...
`.bunnybyte.toml` 默认被 `.gitignore` 忽略，不要把真实 key 提交进 git。
...
如果使用第三方 OpenAI-compatible 中转，`base_url` 写中转地址即可：
...
当前 `protocol = "openai"` 走的是 OpenAI Responses API，也就是会请求
`/v1/responses`。如果中转只支持传统 `/v1/chat/completions`，可能会出现空响应或
格式不兼容。遇到这种情况，优先确认中转是否支持 Responses API，并使用中转后台明确列出的模型名。
...
```bash
export BUNNYBYTE_API_KEY=sk-...
export BUNNYBYTE_BASE_URL=https://api.openai.com/v1
export BUNNYBYTE_MODEL=gpt-5.4
```
...
```bash
bunnybyte --provider openai --model gpt-5.4 --base-url https://api.openai.com/v1
bunnybyte --provider deepseek --approval ask --max-steps 80
bunnybyte --config /path/to/custom.toml --cwd /path/to/repo
```
...
```bash
bunnybyte                              # 默认 Textual TUI
bunnybyte --repl                       # 普通终端 REPL
bunnybyte "找出测试失败的根因"          # one-shot 任务
bunnybyte --resume latest              # 续接最近 session
bunnybyte --cwd /path/to/repo          # 指定工作目录
```

常用运行参数：

```bash
bunnybyte --approval ask               # shell / 写文件前询问
bunnybyte --approval auto              # 普通操作自动通过
bunnybyte --approval never             # 非交互模式
bunnybyte --sandbox best_effort        # 尽量隔离 shell 命令
bunnybyte --no-auto-dream              # 关闭后台 memory 整合
```

如果你想做成类似 `claude` 那样的全局命令，可以创建一个 wrapper：

```bash
cat > ~/.local/bin/bunny <<'EOF'
#!/usr/bin/env bash
exec uv run --project /path/to/Bunny-Byte bunnybyte --cwd "$PWD" "$@"
EOF
chmod +x ~/.local/bin/bunny
```

这样在任意项目目录输入 `bunny`，都会以当前目录作为工作区打开 TUI。
...
| 入口 | 内容 |
| --- | --- |
| [配置](docs/configuration.md) | provider profile、`.bunnybyte.toml`、环境变量和 sandbox 配置。 |
