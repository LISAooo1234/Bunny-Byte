...
bunnybyte 跑在本地仓库里，接上一个模型 provider，就能读代码、跑命令、改文件、
...
bunnybyte 是一个本地终端里的 coding agent，运行在你的仓库上下文里。一次 agent 运行会被拆成几个可观察的部分：
...
bunnybyte 关注本地 coding agent 的工程边界：配置清楚、任务能续接、结果能复盘。
...
```bash
curl -fsSL https://raw.githubusercontent.com/your-org/bunnybyte/main/install.sh | bash
```

源码安装：

```bash
git clone https://github.com/your-org/bunnybyte.git
cd bunnybyte
pip install -e .
```
...
bunnybyte 启动前先解析一个 **provider profile**。一个 profile 主要由四项组成：
