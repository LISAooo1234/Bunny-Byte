...
You are performing a dream: a reflective pass over BunnyByte's memory files. Synthesize recent signal into durable, well-organized memory files so future sessions can orient quickly.
...
def build_dream_retry_prompt(memory_dir):
    return f"""# Dream Retry

You are still inside the same dream consolidation run for `{Path(memory_dir)}`.

Important clarification:
- `write_file` and `patch_file` are allowed in this run.
- Writes must stay inside `{Path(memory_dir)}`.
- The runtime supports multiple sequential tool turns. Emit one tool call at a time and continue until the memory update is complete.
- Do not stop with an explanation that you are blocked or that you need write tools. If a memory update is warranted, perform it now.
- If you need to update an existing file such as `{ENTRYPOINT_NAME}`, first issue a fresh `read_file` for that file in this dream run.
- After reading an existing file, prefer `patch_file` to update it. Use `write_file` for new files that do not exist yet.

Next action:
- Read existing memory files you need to modify, then patch or write the needed files directly in `{Path(memory_dir)}`.
- Then return a short final summary of what changed.
"""
...
def run_dream(agent, quiet=False, session_ids=None):
    from ..core.runtime import BunnyByte
    ...
    if not changed_files and _looks_like_false_dream_block(result):
        result = dream_agent.ask(build_dream_retry_prompt(agent.memory_dir))
        changed_files = _changed_memory_files(before_snapshot, _memory_file_snapshot(agent))
...
def _looks_like_false_dream_block(result):
    text = str(result or "").strip().lower()
    if not text:
        return False
    markers = (
        "blocked:",
        "need to use write tools",
        "permits returning one tool call or one final answer",
        "next concrete update would be to create",
    )
...
def maintain_memory_after_turn(agent, final_answer):
    ...
    thread = threading.Thread(target=_background_dream, name="bunnybyte-auto-dream", daemon=True)
...
def normalize_memory_state(state, workspace_root=None):
    ...
    durable_root = Path(workspace_root) / ".bunnybyte" / "memory" if workspace_root is not None else None
...
def retrieval_candidates(state, query, limit=3, workspace_root=None):
    ...
    if workspace_root is not None:
        durable_store = DurableMemoryStore(Path(workspace_root) / ".bunnybyte" / "memory")
...
class LayeredMemory:
    def __init__(self, state=None, workspace_root=None):
        ...
        self.durable_store = DurableMemoryStore(Path(workspace_root) / ".bunnybyte" / "memory") if workspace_root is not None else None
