...
BunnyByte owns session state, workspace context, memory, checkpoints, and persistence.
...
from ..branding import DISPLAY_NAME, SUBTITLE
...
SHORT_APPROVAL_REPLIES = frozenset(
    {
        "yes",
        "ok",
        "okay",
...
NEXT_STEP_CUE_PATTERN = re.compile(
    r"(?i)(next step|continue|if you'd like|if you want|i can|i will|i'll|接下来|继续|下一步|如果你愿意|我会)"
)
...
class BunnyByte(SessionStateMixin, RuntimeSecretsMixin, RuntimeCheckpointsMixin):
    ...
    def _resolve_memory_dir(self, memory_dir):
        if memory_dir:
            ...
            path = self.root / ".bunnybyte" / "memory"
    ...
    def build_prefix(self):
        ...
        if native_tools_enabled:
            ...
            - Never invent tool results."""
            examples_section = f"Legacy text-protocol examples:\n{legacy_examples}"
        # prefix 可以理解成 agent 的“工作手册”：
        # 它是谁、工具怎么调用、当前仓库是什么状态，都写在这里。
        text = textwrap.dedent(
            f"""\
            You are {DISPLAY_NAME}, a {SUBTITLE} working inside a local repository.
            ...
            - If the user approves a proposed next step, execute it immediately.
            - Treat short replies such as "yes", "ok", "go ahead", "continue", "好", "继续", and "我愿意" as approval to proceed with the latest clear next step.
            - Do not ask whether to continue routine reading, investigation, implementation, or verification when the next step is already clear.
            - Do not end a turn by offering to continue ordinary work later. Perform that work now.
            - Do not present option menus or multiple directions unless the user explicitly asks for options or a real blocking ambiguity remains.
            - Before claiming that a file, symbol, behavior, or fact is missing, search the workspace and read the relevant sources.
            - For multi-step tasks, use the todo tools to track progress and keep statuses current.
            - Before returning a final answer, verify the result with an appropriate check whenever possible.
            - Do not return an acknowledgement-only final such as "ok", "好的", or "done" after taking a tool step. Final answers must contain concrete findings, progress, or a specific blocker.
            - If something could not be verified, say exactly what was not verified and why.
            - Ask the user only when blocked by missing requirements, permissions, or a meaningful ambiguity that changes the work.
    ...
    def effective_user_message(self, user_message):
        text = str(user_message or "")
        normalized = " ".join(text.strip().split()).lower()
        if normalized not in SHORT_APPROVAL_REPLIES:
            return text

        prior_message = self.latest_assistant_message()
        if not prior_message or not NEXT_STEP_CUE_PATTERN.search(prior_message):
            return text

        return textwrap.dedent(
            f"""\
            The user's latest reply was a short approval: "{text.strip()}".
            Interpret it as approval to continue the latest clear next step from the immediately preceding assistant message.
            Continue that work now instead of asking again or returning an acknowledgement-only final.
            Relevant prior assistant message:
            {clip(prior_message, 900)}
            """
        ).strip()
