"""Read-only helpers for BunnyByte run/session evidence.

These helpers intentionally do not import or instantiate the BunnyByte runtime. They
only interpret files that a completed CLI/REPL/TUI run left under `.bunnybyte/`.
"""
...
class RunEvidence:
    ...
    def latest(cls, workspace: Path) -> "RunEvidence":
        ...
        run_dir = _latest_dir(workspace / ".bunnybyte" / "runs")
        ...
        session_path = _latest_file(workspace / ".bunnybyte" / "sessions", "*.json")
        session_event_path = _latest_file(workspace / ".bunnybyte" / "sessions", "*.events.jsonl")
        return cls(
            workspace=workspace,
            run_dir=run_dir,
