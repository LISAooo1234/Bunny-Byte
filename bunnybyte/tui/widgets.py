...
class WelcomeBanner(Static):
    ...
    WelcomeBanner {{
        ...
        margin: 1 1 0 1;
        padding: 1 2;
        background: {TUI_PANEL_BG};
        color: {TUI_TEXT};
        border: round {TUI_PANEL_BORDER};
    }}
    """
        ...
        for pixel_row in mascot_stacked_rows():
            ...
            for top_color, bottom_color in pixel_row:
                if top_color is None and bottom_color is None:
                    ...
                    line.append(LOWER_HALF_BLOCK, style=bottom_color)
...
class InputBar(Static):
    ...
    def selected_slash_suggestion(self) -> SlashCommand | None:
        if not self._slash_suggestions:
            return None
        return self._slash_suggestions[self._slash_index]
